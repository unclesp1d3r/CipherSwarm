...
CipherSwarm **production** runs in **air-gapped, non-Internet-connected lab environments**. Development and testing happen on Internet-connected machines — the air-gap constraint applies only to the deployed production system. Every architectural, dependency, and infrastructure decision must be evaluated against these production constraints:

- **No Internet access in production** — no CDN assets, no external API calls, no package fetching at runtime, no cloud-hosted services. All dependencies must be vendored or bundled into the Docker image at build time. Docker images are built on connected machines and transferred to the lab.
...
**When making decisions, always ask:** "Will this work in production on an isolated LAN with no Internet, 10+ agents, and 100 GB files?"
