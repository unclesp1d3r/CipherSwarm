...
- **High-frequency broadcasts must be async AND throttled.** Use `broadcast_replace_later_to` (Sidekiq) — never the synchronous `broadcast_replace_to` — for anything that fires on `Task` transitions, `HashItem` cracks, or other per-event callbacks. Wrap the broadcast in `throttled_broadcast(key, ttl:)` from `SafeBroadcasting` so a burst collapses to one frame per record per window. Pass a per-record key (`"<concept>_#{id}"`); the helper automatically namespaces it with `THROTTLE_KEY_PREFIX` (`"throttle:broadcast"`) to avoid collision with application-level `Rails.cache` entries.
- **Leading-edge vs trailing-edge.** Use *leading-edge* (`throttled_broadcast` + `broadcast_replace_later_to`) for continuous-value displays like progress bars and ETA — keeps the UI live during a burst, accepts up to one window of staleness at the tail. Use *trailing-edge* (`Rails.cache.write(...)` gate + `Job.set(wait: window).perform_later`, see `HashItem#enqueue_recent_cracks_broadcast`) for snapshot panels where only the final state matters — collapses the burst tail into a single delayed render.
- **`throttled_broadcast` fails open in two distinct paths.** (1) If `Rails.cache.write` raises (Redis outage), the block still yields — preferable to silencing all UI updates. (2) If the yielded block itself raises *after* the key was set, the helper deletes the key before re-raising so the next caller is not silently suppressed for the remainder of the TTL. Without this second path, a single render bug would cause a 5s blackout per record.
- **`throttled_broadcast` mirrors `BROADCAST_METHODS` rescue posture.** Connection-class errors (`EXPECTED_BROADCAST_ERRORS` — `IOError`, `Errno::ECONNREFUSED/RESET/EPIPE`, `Redis::BaseError`) fail open silently. Other `StandardError`s fail open in production but re-raise in development so bugs surface during a normal `bin/dev` run.
- **State-machine `after_transition` callers must use a `safe_*` wrapper.** The `state_machines` gem treats unrescued exceptions in `after_transition` as a rollback signal — a render bug inside `throttled_broadcast` would silently revert a legitimate state move. See `Attack#safe_broadcast_attack_progress_update` and `Task#safe_broadcast_attack_progress_update` for the pattern.
- **State changes + `after_commit` can double-trigger broadcasts.** Models that fire `broadcast_*` from both an `after_transition` (state machine) and an `after_commit on: [:update]` will hit both callbacks for every state change. Guard the `after_commit` with `unless: -> { saved_change_to_state? }` so the after_transition is the sole trigger for state moves while after_commit still covers non-state-change updates (counter cache touches, attribute edits). The throttle would absorb a double-fire, but the redundant `SET NX EX` is measurable under high concurrent agent load.
- **Sidekiq enqueue failure inside a throttled block.** If `broadcast_replace_later_to` raises after the throttle key was set, the helper's block-rescue deletes the key (see fail-open path 2 above), so the next call re-fires within the same window. Without that release path, the throttle would absorb the rest of the TTL window with no broadcast actually reaching the UI.
- **Dev `null_store` silently disables throttling.** When `REDIS_URL` is unset in development, `config/environments/development.rb` defaults to `:null_store`, where every `Rails.cache.write` returns `true` regardless of `unless_exist:`. Throttling never suppresses; every callback fires an unthrottled broadcast. Not a correctness bug — broadcasts are still async — but the dev path does not exercise the production suppression semantic. Set `REDIS_URL=redis://localhost:6379/0` locally to mirror production behavior.
