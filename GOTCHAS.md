...
- **High-frequency broadcasts must be async AND throttled.** Use `broadcast_replace_later_to` (Sidekiq) — never the synchronous `broadcast_replace_to` — for anything that fires on `Task` transitions, `HashItem` cracks, or other per-event callbacks. Wrap the broadcast in `throttled_broadcast(key, ttl:)` from `SafeBroadcasting` so a burst collapses to one frame per record per window. Throttle keys are per-record (`"<concept>_#{id}"`); each record gets its own window.
- **Leading-edge vs trailing-edge.** Use *leading-edge* (`throttled_broadcast`, `Rails.cache.write(..., unless_exist: true)` followed by an immediate `broadcast_replace_later_to`) for continuous-value displays like progress bars and ETA — keeps the UI live during a burst, accepts up to one window of staleness at the tail. Use *trailing-edge* (`Rails.cache.write(...)` gate + `Job.set(wait: window).perform_later`, see `HashItem#enqueue_recent_cracks_broadcast`) for snapshot panels where only the final state matters — collapses the burst tail into a single delayed render.
- `throttled_broadcast` fails open: if `Rails.cache.write` raises (Redis outage), the block still yields. Suppressing all broadcasts on a cache outage is worse than the occasional unthrottled frame.
