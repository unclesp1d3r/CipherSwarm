...
async def submit_task_result_v1(
    ...
    try:
        await submit_task_result_service(id, data, db, authorization)
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    except InvalidAgentTokenError as e:
        raise HTTPException(status_code=401, detail="Not authorized") from e
    except TaskNotFoundError as e:
...
async def submit_task_status_v1(
    ...
    try:
        result = await update_task_progress_service(id, data, db, authorization)
        if result == "stale":
...
async def submit_cracked_hash_v1(
    id: Annotated[int, Path(alias="id")],  # noqa: A002,ARG001
    data: HashcatResult,  # noqa: ARG001
    db: Annotated[AsyncSession, Depends(get_db)],  # noqa: ARG001
    authorization: Annotated[str, Header(alias="Authorization")],  # noqa: ARG001
) -> JSONResponse:
