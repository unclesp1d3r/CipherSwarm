...
async def agent_heartbeat(
    ...
    try:
        await heartbeat_agent_service_v2(request, data, db, authorization, user_agent)
    except (InvalidUserAgentError, InvalidAgentTokenError) as e:
...
async def update_agent_state(
    ...
    try:
        await update_agent_state_service_v2(data, db, authorization, user_agent)
    except (InvalidUserAgentError, InvalidAgentTokenError) as e:
        raise HTTPException(status_code=401, detail=str(e)) from e
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def update_task_progress(
    ...
    try:
        await update_task_progress_service_v2(
            task_id, data, db, authorization, user_agent
        )
    except (InvalidUserAgentError, InvalidAgentTokenError) as e:
        raise HTTPException(status_code=401, detail=str(e)) from e
...
async def submit_task_result(
    ...
    try:
        await submit_task_result_service_v2(
            task_id, data, db, authorization, user_agent
        )
    except (InvalidUserAgentError, InvalidAgentTokenError) as e:
        raise HTTPException(status_code=401, detail=str(e)) from e
...
@router.get(
    ...
    description="Request a new task from the server, if available. Requires valid Authorization and User-Agent headers.",
)
async def get_new_task(
    ...
    try:
        task = await get_new_task_service_v2(db, authorization, user_agent)
        return JSONResponse(
...
@router.post(
    ...
    description="Submit a cracked hash result for a task. Requires valid Authorization and User-Agent headers.",
)
async def submit_cracked_hash(
    ...
    try:
        result = await submit_cracked_hash_service_v2(
            task_id, data, db, authorization, user_agent
        )
