...
module TaskStateMachine
  ...
  included do
    state_machine :state, initial: :pending do
      ...
      after_transition on: :retry do |task|
        ...
        task.update_columns(retry_count: task.retry_count + 1, last_error: nil, cached_progress_pct: 0.0)
      ...
      after_transition on: :preempt do |task|
        ...
        # rubocop:disable Rails/SkipsModelValidations
        task.update_columns(stale: true, preemption_count: task.preemption_count + 1, cached_progress_pct: 0.0)
        # rubocop:enable Rails/SkipsModelValidations
      end
  ...
  def mark_stale_safely
    ...
    # rubocop:disable Rails/SkipsModelValidations
    update_columns(stale: true, cached_progress_pct: 0.0)
    # rubocop:enable Rails/SkipsModelValidations
