...
class AddPerformanceIndexes < ActiveRecord::Migration[7.2]
  def change
    ...
    add_index :hash_items, %i[hash_value hash_list_id], name: "index_hash_items_on_hash_value_and_hash_list_id"

    # Add composite index for cracked hash lookups (used in ProcessHashListJob)
    add_index :hash_items, %i[hash_value cracked], name: "index_hash_items_on_hash_value_and_cracked"

    # Add index for agent state filtering
    add_index :agents, %i[state last_seen_at], name: "index_agents_on_state_and_last_seen_at"
