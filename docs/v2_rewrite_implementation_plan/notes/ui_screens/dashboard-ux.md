...
- [CipherSwarm v2 Dashboard UX Design](#cipherswarm-v2-dashboard-ux-design)
  ...
  - [Campaign Overview Section](#campaign-overview-section)
    - [Campaign Row Design](#campaign-row-design)
  ...
  - [Technical Notes](#technical-notes)
    - [Real-Time Update Error Handling Implementation](#real-time-update-error-handling-implementation)
...
- **Header:**
  ...
  - Live status indicators (real-time connection, backend status)
...
- **Main Content Area:**
  ...
  - Top strip with operational status **cards** arranged in a responsive row layout. Each card summarizes a critical system metric and uses a compact, visually scannable format. All cards should update reactively using real-time data, with no need for manual refresh.
    ...
    - **Resource Usage** — Shows a lightweight visual (e.g., sparkline or mini line chart) of aggregate hash rate (hashes per second) across all agents over the last 8 hours. Data comes from agent device performance timeseries.

Each card should follow semantic layout patterns with proper component structure (header, title, content, etc.) and use consistent spacing and responsive design. Avoid over-styling; favor clarity and clean design principles.
...
- Accordion/collapsible component for campaign rows
...
- Uses standard toast/notification component
- Option to view full hashlist page or suppress future toasts

## Agent Status Overview

Agent status is accessible via a slide-out panel anchored to the right side of the dashboard. The panel is triggered by clicking the **Active Agents** card in the top strip.

Each agent is displayed as a vertically stacked card inside the panel:
...
The panel is full-height and fixed-width with a scrollable vertical layout. It is not mobile-optimized. This approach allows a scalable, glanceable agent view without cluttering the main dashboard.
...
This is purely illustrative and should be translated into a real card component with dynamic values and styling.
...
- All status data must update via real-time streaming connections.
- If real-time updates are unavailable, provide manual refresh option instead of automatic polling (chosen implementation).
- Show stale data indicators if last update >30 seconds with "Refresh Now" button.
- Toast notification displayed when real-time connection exhausts reconnection attempts: "Real-time updates disconnected. Please refresh the page."
- Must work offline for development (mock data stream).

### Real-Time Update Error Handling Implementation

The dashboard implements a user-controlled approach to connection failures rather than automatic background polling:

- **Automatic Reconnection**: Real-time service attempts 5 reconnections with exponential backoff
...
- Layout: Sidebar, Header, Accordion, Toast, Progress, Tooltip, Dialog/Modal
- Typography: Heading, Text, Badge
- UI logic: State management for session and real-time updates, role-aware navigation
...
- All live data (campaigns, agents, cracked hashes) is streamed via real-time connections
- Frontend should use appropriate state management to handle reactive updates
