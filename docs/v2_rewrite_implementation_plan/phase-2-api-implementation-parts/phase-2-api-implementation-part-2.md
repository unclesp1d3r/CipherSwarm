...
-   [ ] Add confirmation/preview screen before launching a generated campaign to the crackable upload UI (implemented in `frontend/`). Shows detected hash type, parsed sample lines, and proposed attacks/resources `task_id:upload.preview_summary_ui` - (check `docs/v2_rewrite_implementation_plan/notes/user_flows_notes.md` and `docs/v2_rewrite_implementation_plan/side_quests/salvage_templates.md` for the expected structure and UI)
