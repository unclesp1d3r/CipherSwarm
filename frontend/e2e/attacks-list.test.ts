...
test.describe('Attacks List Page', () => {
    ...
    test('should load and display attacks correctly', async ({ page }) => {
        ...
        await expect(page.getByTestId('attack-row-1').locator('[data-slot="badge"]').getByText('Dictionary')).toBeVisible();
        await expect(page.getByTestId('attack-row-2').locator('[data-slot="badge"]').getByText('Brute Force')).toBeVisible();
        await expect(page.getByTestId('attack-row-3').locator('[data-slot="badge"]').getByText('Mask')).toBeVisible();
    ...
    test('should handle empty state correctly', async ({ page }) => {
        ...
        await page.route('**/api/v1/web/attacks*', async (route) => {
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify(emptyAttacksResponse)
            });
        });
    ...
    test('should display and interact with attack action menus', async ({ page }) => {
        ...
        // Should reload attacks (we can't easily test this without complex mocking)
    });
    ...
    test('should display loading skeleton correctly', async ({ page }) => {
