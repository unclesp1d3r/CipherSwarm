...
test.describe('Crackable Upload Modal', () => {
	...
	test.describe('Upload Operations', () => {
		test('uploads text content successfully', async ({ page }) => {
			// Mock hash types endpoint
			await page.route(/\/api\/v1\/web\/modals\/hash_types/, async (route) => {
				if (route.request().method() === 'GET') {
					await route.fulfill({
						status: 200,
						contentType: 'application/json',
						body: JSON.stringify([
							{ mode: 1000, name: 'NTLM', confidence: 0.95 },
							{ mode: 1800, name: 'SHA512crypt', confidence: 0.85 }
						])
					});
				}
			});

			// Mock upload response with delay to show loading state
			await page.route(/\/api\/v1\/web\/uploads\/$/, async (route) => {
				if (route.request().method() === 'POST') {
			...
			// Mock upload status response for preview
			await page.route(/\/api\/v1\/web\/uploads\/123\/status/, async (route) => {
				if (route.request().method() === 'GET') {
					await route.fulfill({
						status: 200,
						contentType: 'application/json',
						body: JSON.stringify({
							...
							processing_steps: [
								{ step_name: 'upload', status: 'completed', progress: 100 },
								{ step_name: 'analysis', status: 'completed', progress: 100 }
							],
							campaign_id: 456,
							hash_list_id: 789,
			...
			await expect(page.locator('text=Preview & Launch Campaign')).toBeVisible();

			// Verify preview content is shown
			await expect(page.locator('text=NTLM').first()).toBeVisible();
			await expect(page.locator('text=Showing 1 of 1')).toBeVisible();

			// Wait for launch button to be enabled
			await expect(
				page.locator('[data-testid="launch-campaign-button"]:not([disabled])')
			).toBeVisible();
		...
		test('uploads file successfully', async ({ page }) => {
			...
			// Mock upload status response for preview
			await page.route(/\/api\/v1\/web\/uploads\/456\/status/, async (route) => {
				if (route.request().method() === 'GET') {
					await route.fulfill({
						status: 200,
						contentType: 'application/json',
			...
			// Verify preview content is shown
			await expect(page.locator('text=SHA512crypt').first()).toBeVisible();
			await expect(page.locator('text=Showing 1 of 1')).toBeVisible();
	...
	test.describe('Integration with Campaigns Page', () => {
		test('refreshes campaigns list after successful upload', async ({ page }) => {
			...
			// Mock upload status response for preview
			await page.route(/\/api\/v1\/web\/uploads\/789\/status/, async (route) => {
				if (route.request().method() === 'GET') {
					await route.fulfill({
						status: 200,
						contentType: 'application/json',
