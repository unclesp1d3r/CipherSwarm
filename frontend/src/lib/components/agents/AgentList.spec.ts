...
describe('AgentList', () => {
    it('renders table headers', async () => {
        ...
        expect(await screen.findByText((content, node) => node?.tagName === 'TH' && /Status/.test(content))).toBeInTheDocument();
        expect(await screen.findByText((content, node) => node?.tagName === 'TH' && /Temperature/.test(content))).toBeInTheDocument();
        expect(await screen.findByText((content, node) => node?.tagName === 'TH' && /Utilization/.test(content))).toBeInTheDocument();
        expect(await screen.findByText((content, node) => node?.tagName === 'TH' && /Current Attempts\/sec/.test(content))).toBeInTheDocument();
        expect(await screen.findByText((content, node) => node?.tagName === 'TH' && /Average Attempts\/sec/.test(content))).toBeInTheDocument();
