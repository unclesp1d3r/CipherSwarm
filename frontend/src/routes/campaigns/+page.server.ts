        ...
        const campaignsWithAttacks = await Promise.all(
            campaignsResponse.items.map(async (campaign) => {
                try {
                    ...
                    const completedAttacks = attacks.filter(attack =>
                        attack.attack_mode === 'completed' // This might need adjustment based on actual attack state
                    ).length;
                    const progress = totalAttacks > 0 ? (completedAttacks / totalAttacks) * 100 : 0;
                    const summary = totalAttacks > 0
                    ...
                    console.warn(`Failed to fetch attacks for campaign ${campaign.id}:`, attackError);
