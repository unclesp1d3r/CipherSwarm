...
test.describe('Authentication Flow', () => {
	...
	test('should login successfully with regular user credentials', async ({ page }) => {
		...
		// Should see user info in navigation
		await expect(page.locator('[data-testid="user-menu"]')).toContainText(TEST_USERS.user.name);
	});
	...
	test('should handle concurrent sessions correctly', async ({ browser }) => {
		...
		try {
			...
			await expect(page1.locator('[data-testid="user-menu"]')).toContainText(
				TEST_USERS.admin.name
			);
			await expect(page2.locator('[data-testid="user-menu"]')).toContainText(
				TEST_USERS.user.name
			);
		} finally {
