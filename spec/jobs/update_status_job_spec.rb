    ...
    context 'when the task has been running for more than 30 minutes' do
      ...
      it 'abandons the task' do
        expect { described_class.new.perform }
          .to change { running_task.reload.state }.from('running').to('failed')
      end

      it 'does not abandon the task' do
        expect { described_class.new.perform }
          .not_to change { not_running_task.reload.state }
      end
    end

    context 'when the task has hashcat statuses' do
      let(:running_task) { create(:task, state: :running, attack:, agent:,
                                         hashcat_statuses: build_list(:hashcat_status, 50)) }

      let(:exhausted_task) { create(:task, state: :exhausted, attack:, agent:,
                                           hashcat_statuses: build_list(:hashcat_status, 50)) }

      let(:pending_task) { create(:task, state: :pending, attack:, agent:,
                                         hashcat_statuses: build_list(:hashcat_status, 50)) }

      let(:completed_task) { create(:task, state: :completed, attack:, agent:,
                                           hashcat_statuses: build_list(:hashcat_status, 50)) }

      it 'does not remove the hashcat statuses of the running task' do
        expect { described_class.new.perform }
          .not_to change { running_task.hashcat_statuses.count }.from(50)
      end

      it 'removes the hashcat statuses of the exhausted task' do
        expect { described_class.new.perform }
          .to change { exhausted_task.hashcat_statuses.count }.from(50).to(10)
      end

      it 'does not remove the hashcat statuses of the pending task' do
        expect { described_class.new.perform }
          .not_to change { pending_task.hashcat_statuses.count }.from(50)
      end

      it 'does remove the hashcat statuses of the completed task' do
        expect { described_class.new.perform }
          .to change { completed_task.hashcat_statuses.count }.from(50).to(10)
      end
    end
  end
end
