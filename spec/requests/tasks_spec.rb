...
RSpec.describe "Tasks" do
  ...
  describe "GET /tasks/:id/download_results" do
    context "when a project member is logged in" do
      it "returns CSV with correct headers and filename" do
        create(:hash_item, :cracked_recently,
      ...
      it "includes only cracked hashes in CSV" do
        cracked = create(:hash_item, :cracked_recently,
                         hash_list: task.hash_list,
                         hash_value: "cracked123",
                         plain_text: "password1")
        create(:hash_item,
               hash_list: task.hash_list,
        ...
        expect(csv_rows.first["Hash"]).to eq(cracked.hash_value)
        expect(csv_rows.pluck("Hash")).not_to include("uncracked789")
      end

      it "redirects to task when requesting HTML format" do
        sign_in(project_user)
        get download_results_task_path(task, format: :html)
