...
class AttackResourceFormPage < BasePage
  ...
  def has_project_checkboxes?
    # SimpleForm generates checkboxes with input[type="checkbox"] for check_boxes
    # Radio buttons would be input[type="radio"]
    session.has_css?("input[type='checkbox'][name*='project_ids']")
  end

  # Check if project radio buttons are displayed (should be false after fix)
  # @return [Boolean] true if radio buttons are present
  ...
  def available_projects
    session.all("input[type='checkbox'][name*='project_ids']").map do |checkbox|
      # SimpleForm wraps checkboxes in labels, find the label using the checkbox ID
