...
RSpec.describe "Create rule list" do
  ...
  describe "create rule list with single project" do
    before do
      user.projects << [first_project, second_project]
    end

    it "creates rule list associated with one project" do
      visit new_rule_list_path

      form_page.fill_name("Test Rule List")
      ...
      expect(page).to have_content("Rule list was successfully created", wait: 5)

      rule_list = RuleList.find_by(name: "Test Rule List")
      expect(rule_list).to be_present
      expect(rule_list.projects).to contain_exactly(first_project)
      expect(rule_list.sensitive).to be true
    end
  end

  describe "create rule list with multiple projects" do
    before do
      user.projects << [first_project, second_project]
    end

    it "creates rule list associated with multiple projects" do
      visit new_rule_list_path

      form_page.fill_name("Multi-Project Rule List")
      form_page.fill_description("Test description")
      form_page.attach_file(file_path)
      form_page.wait_for_upload_complete
      ...
      rule_list = RuleList.find_by(name: "Multi-Project Rule List")
      expect(rule_list).to be_present
      expect(rule_list.projects).to contain_exactly(first_project, second_project)
      expect(rule_list.sensitive).to be true
    end
  end

  describe "create rule list without project selection" do
    before do
      user.projects << [first_project, second_project]
    end

    it "creates shared rule list when no projects selected" do
      visit new_rule_list_path

      form_page.fill_name("Shared Rule List")
      form_page.fill_description("Test description")
      form_page.attach_file(file_path)
      form_page.wait_for_upload_complete
      form_page.submit_form
      ...
      rule_list = RuleList.find_by(name: "Shared Rule List")
      expect(rule_list).to be_present
      expect(rule_list.projects).to be_empty
      expect(rule_list.sensitive).to be false
    end
  end
  ...
  describe "project visibility based on user membership" do
    ...
    context "when user is member of project" do
      ...
      it "allows creating resource with accessible project" do
        ...
        form_page.fill_name("Accessible Project Rule List")
        form_page.fill_description("Test description")
        form_page.attach_file(file_path)
        form_page.wait_for_upload_complete
        ...
        rule_list = RuleList.find_by(name: "Accessible Project Rule List")
        expect(rule_list).to be_present
        expect(rule_list.projects).to contain_exactly(accessible_project)
      end
    end
    ...
    context "when user has no project memberships" do
      ...
      it "allows creating shared resource without projects" do
        ...
        rule_list = RuleList.find_by(name: "Shared Resource No Projects")
        expect(rule_list).to be_present
        expect(rule_list.projects).to be_empty
        expect(rule_list.sensitive).to be false
      end
